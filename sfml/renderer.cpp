...
void Renderer::updateShapes(Game* pGame) {
	...
	// Load texture
	if (!ball_texture.loadFromFile("Assets/Sprites/ball_red.png"))
	{
	...
	// Let's do the ground with some more graphics shapes
	box2.setSize(sf::Vector2f(pGame->myBox2.getWidth(), pGame->myBox2.getHeigth()));
	box2.setPosition(pGame->myBox2.getPos());
	box2.setFillColor(sf::Color::White);


	// Let's do the ground with some more graphics shapes
	box3.setSize(sf::Vector2f(pGame->myBox3.getWidth(), pGame->myBox3.getHeigth()));
